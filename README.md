...
This repo contains all the code of the website from https://animalshelter.com
...
1. Check that you have your .env.dev and .env.dev.db.  If for some reason these files are not generated, remember that you can find samples in the repository.
   ...
   SQL_DATABASE=animal-shelter_dev
   SQL_USER=animal-shelter
   SQL_PASSWORD=animal-shelter
   SQL_HOST=db
   SQL_PORT=5432
   DATABASE=postgres
   ...
   POSTGRES_USER=animal-shelter
   POSTGRES_PASSWORD=animal-shelter
   POSTGRES_DB=animal-shelter_dev
    ```

2. Create docker volumes to save the data:
    ```sh
    $ docker volume create --name=shelter_postgres_data_dev
 ...
 * **Environment files:** Once the server is created, you must copy the .env files in the production server inside **/opt/shelter** path.
    ```sh
    $ cat .env.prod
    $ cat .env.prod.db
